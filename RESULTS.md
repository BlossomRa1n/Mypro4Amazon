...
## 6. 融合权重（grid search 结果）

两阶段 grid search（10 万 val 用户，Stage 1 四路 × 540 组合，Stage 2 HSTU×MIND × 16 组合）：

| 配置 | 权重 | 加权融合 HR@100 |
|---|---|---|
| 旧四路 | itemcf=1.5, v2=1.0, cat=0.7, hot=0.05 | 13.15% |
| **新四路最优** | itemcf=1.5, **v2=3.0**, **cat=1.0**, hot=0.05 | **13.76%** (+0.61) |
| 六路最优 | 上述 +hstu=0.5, mind=0.3 | 13.58% (-0.17) |

**关键发现一（权重漂移）**：Phase 2 使 V2 从 8.66% → 10.13%，最优权重里 V2 从 1.0 升到 **3.0**、category 从 0.7 升到 1.0，四路加权融合 13.15% → **13.76%**。

**关键发现二（加权融合兑现不了并集增益）**：六路加权融合 13.58% **反而低于**四路 13.76%，尽管六路并集天花板高达 22.02%。原因是 HSTU / MIND 的独有命中（+3.26pp / +1.59pp）分散在各路 top-100 里，而 `merge_fast` 的加权 quota 只给 HSTU 7 个、MIND 4 个名额，绝大部分独有命中被稀释丢弃。

**结论**：HSTU / MIND 的并集价值真实（22.02% vs 17.53%），但**当前"加权求和 + 单一 top-100"的融合方式兑现不了**，需要按独有命中分配 quota / 并集去重等更强的融合策略（见 FUTURE_WORK）。当前生产建议先落地**新四路权重 13.76%**。
