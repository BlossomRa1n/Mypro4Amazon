...
> **并集 vs 加权融合**：上表是**并集天花板**（各路 top-100 全量合并的理论上界）。当前生产用的"加权求和 + 单一 top-100"实际达到**四路 13.76%**（`itemcf=1.5, v2=3.0, cat=1.0, hot=0.05`）；六路加权反而 13.58%，因为 quota 稀释了 HSTU/MIND 的独有命中。兑现 22.02% 需要更强的融合策略，详见 [RESULTS.md](RESULTS.md) §6。
