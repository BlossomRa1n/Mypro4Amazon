"""
V2 双塔全库 HR@100 评估 — Round 3+ 实验归因用 (每次 V2 重训后跑, 对标基线 8.66%)。

用法:
  python eval_v2_hr100.py [model_path] [embeddings_pkl]
  默认: model_path=config.V2_BEST_FILE, embeddings_pkl=config.V2_EMBED_PKL

采样 10 万 val 用户, 用 V2 双塔做 top-100 向量召回, 算命中率。
复用 recall_fusion.embedding_recall (user_tower_type 默认 sasrec)。
"""
import os, sys
...
def main():
    model_path = sys.argv[1] if len(sys.argv) > 1 else config.V2_BEST_FILE
    embed_pkl = sys.argv[2] if len(sys.argv) > 2 else config.V2_EMBED_PKL
    label = sys.argv[3] if len(sys.argv) > 3 else "V2"

    t0 = time.time()
    print(f">>> model_path={model_path}")
    print(f">>> embed_pkl={embed_pkl}")
    print(">>> Loading data...")
    ...
    print(f">>> embeddings shape: {all_item_vecs.shape}")
    ...
    print(f">>> {label} embedding recall (top-100)...")
    recall_dict = embedding_recall(
        ...
        model_path=model_path, channel_label=label
    )
    ...
    for u in sample_users:
        ...
        if not gt:
            continue
        pool = set(recall_dict.get(u, {}).keys())
        if gt & pool:
            hit += 1
    hr100 = hit / max(n, 1)
    print("\n" + "=" * 60)
    print(f">>> {label} 全库 HR@100 = {hr100*100:.2f}% ({hit:,}/{n:,})")
    print(f">>> 基线 V2=8.66% | ItemCF=10.62% | 四路并集=16.89%")
