...
| `python code/train_deep.py` | 训练双塔模型 (TwoTowerModel + BPR loss) | 可用 |
| `python code/train_v2.py` | 训练 SASRec 双塔 (TwoTowerV2Model + InfoNCE) | 可用 |
| `python code/train_din.py` | 训练 DIN 精排模型 | 可用 |
...
| `code/train_deep.py` | 双塔模型训练入口：加载数据 → 构建编码器 → 训练 → 保存 checkpoint + item embedding pickle |
| `code/check.py` | 数据校验：读取 Amazon CSV 前 5 行，验证数据文件是否就绪 |
| `code/config.py` | 全局配置：路径、超参数、Amazon 类别选择 |

### Model Evolution (in `model.py`)

1. **TwoTowerModel** (基础双塔) - 用户塔平均池化 + 物品塔 MLP，BPR pairwise loss，输出 L2 归一化 64 维向量
2. **TwoTowerV2Model** (SASRec) - Transformer 因果自注意力用户塔 (2 blocks, 2 heads)，InfoNCE in-batch negative loss + Hard Negative Mining
3. **MINDModel** (多兴趣网络) - 动态路由 (capsule-style, 3 iterations) 将用户历史聚类为 K=3 兴趣胶囊，独立召回后合并
4. **DINModel** (精排模型) - 注意力机制让目标 item 与用户历史动态交互，特征拼接 325 维，sigmoid 输出点击概率，BCE loss
