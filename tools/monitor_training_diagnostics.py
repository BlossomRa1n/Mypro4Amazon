"""Supervise one explicitly supplied diagnostic command; never restart or shut down.

Example: python tools/monitor_training_diagnostics.py --run-dir /evidence/run \
 --control-dir /evidence/control -- python code/run_training_diagnostics.py ...
...
def snapshot(run, process, started, gpu=True):
    now = time.monotonic()
    ...
    return {'timestamp_utc': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'elapsed_seconds': now - started,
            ...
            'gpu': gpu_info, 'latest_steps': {str(path.relative_to(run)): latest_step(path) for path in sorted(run.glob('seed_*/raw/steps.jsonl'))},
...
def supervise(args):
    ...
    atomic_json(control / 'launch.json', {'command': command, 'run_dir': str(run), 'control_dir': str(control), 'interval_seconds': float(args.interval), 'automatic_restart': False, 'shutdown_allowed': False})
        ...
        with (control / 'training.log').open('ab', buffering=0) as log:
            ...
            while True:
                ...
                if code is not None or now >= next_sample:
                    value = snapshot(run, process, started, gpu=not getattr(args, 'no_gpu_probe', False))
